...
<div class="card"><h2>TBB Customer Dashboard</h2><div class="dashcards"><div class="dashstat"><div class="label">Total TBB Customers</div><div class="number">{{ total_customers }}</div></div><div class="dashstat"><div class="label">TBB Customers With Bookings</div><div class="number">{{ customers_with_bookings }}</div></div><div class="dashstat"><div class="label">Total TBB Bookings</div><div class="number">{{ total_tbb_bookings }}</div></div></div><div class="customer-table table"><table><thead><tr><th>Sr.</th><th>TBB Code</th><th>Customer Name</th><th>Mobile No.</th><th>GSTIN No.</th><th>Address</th><th>Created</th></tr></thead><tbody>{% for c in customers %}<tr><td>{{ loop.index }}</td><td class="customer-code">{{ c.code }}</td><td class="customer-name">{{ c.customer_name }}</td><td>{{ c.mobile or '-' }}</td><td>{{ c.gstin or '-' }}</td><td>{{ c.address or '-' }}</td><td>{{ c.created_at or '-' }}</td></tr>{% else %}<tr><td colspan="7" class="empty">No TBB customers added yet.</td></tr>{% endfor %}</tbody></table></div></div>
...
def tbb_customers():
    ...
    customers=[]; total_customers=0; customers_with_bookings=0; total_tbb_bookings=0
    con=db(); cur=con.cursor()
    cur.execute("SELECT * FROM tbb_customers ORDER BY id DESC")
    customers=cur.fetchall()
    total_customers=len(customers)
    cur.execute("SELECT COUNT(*) AS c FROM shipments WHERE UPPER(COALESCE(tbb_customer_code,'')) <> ''")
    total_tbb_bookings=cur.fetchone()['c']
    cur.execute("SELECT COUNT(DISTINCT UPPER(tbb_customer_code)) AS c FROM shipments WHERE UPPER(COALESCE(tbb_customer_code,'')) <> ''")
    customers_with_bookings=cur.fetchone()['c']
    if q:
    ...
    cur.close(); con.close()
    return render_template_string(TBB_PAGE_HTML,q=q,selected=selected,bookings=bookings,stats=stats,customers=customers,total_customers=total_customers,customers_with_bookings=customers_with_bookings,total_tbb_bookings=total_tbb_bookings)
...
def tbb_customer_add():
    ...
    cur.execute("SELECT MAX(CASE WHEN code ~ '^[0-9]+$' AND CAST(code AS BIGINT) >= 202601 THEN CAST(code AS BIGINT) ELSE NULL END) AS n FROM tbb_customers"); row=cur.fetchone(); n=int(row['n']) + 1 if row and row['n'] is not None else 202601; code=str(n)
...
<div class="panel"><h3>Shipment Management</h3><div class="filters"><input id="search" placeholder="Search GR / consignor / consignee / status..."></div><div class="tablewrap"><table><thead><tr><th>Sr.</th><th>GR No.</th><th>Booking Date</th><th>Consignor</th><th>Consignee</th><th>Route</th><th>Charge Weight</th><th>Packages</th><th>Status</th><th>Action</th></tr></thead><tbody>{% for s in shipments %}<tr><td>{{ loop.index }}</td><td><b>{{ s.docket }}</b></td><td>{{ s.booking_date }}</td><td>{{ s.consignor or '-' }}</td><td>{{ s.consignee or '-' }}</td><td>{{ s.source }} → {{ s.destination }}</td><td>{{ s.charge_weight or s.weight or '-' }}</td><td>{{ s.packages or 0 }}</td><td>{{ s.status }}</td><td><a href="{{ url_for('edit_shipment',sid=s.id) }}">Edit</a> · <a href="{{ url_for('shipment_pdf',sid=s.id) }}" target="_blank">GR PDF</a> · <form method="POST" action="{{ url_for('delete_shipment',sid=s.id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete GR No. {{ s.docket }}? This booking and its tracking history will be permanently deleted.');"><button type="submit" style="border:0;background:none;padding:0;color:#b00000;text-decoration:underline;cursor:pointer;font:inherit;">Delete</button></form></td></tr>{% endfor %}</tbody></table></div></div></main>
